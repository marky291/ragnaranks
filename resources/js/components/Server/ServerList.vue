            ...
            <div class="mb-3 server-card item flex-fill tw-shadow border rounded">
                    ...
                    <div class="tw-flex">
                        <div class="left-side d-flex w-75 flex-column px-4 py-2 align-self-end">
                            ...
                            <ul class="tag-list tw-flex tw-flex-wrap tw-text-xs tw-text-green-light" style="font-size:13px; margin-bottom: .5rem; width:inherit">
                                <li v-for="tag in listing.tags" :key="tag.id" class="mr-2">#{{ tag }}</li>
                            </ul>
                        </div>
                ...
                <div class="server-card-body align-items-center tw-shadow-inner px-4 py-3 d-flex">
                    ...
                    <div class="tw-border-l-2 tw-pl-4 tw-pr-5 tw-border-grey-light">
                        <p class="tw-text-gray-700 tw-tracking-tighter tw-font-semibold mb-0" style="font-size:14px">{{ listing.rate.label }}</p>
                    ...
                    <div class="tw-flex-1 tw-pr-12 tw-py-1 tw-flex tw-leading-tight tw-text-gray-700" style="font-size:8px; justify-content: space-evenly; border-left: 1px dashed #cacaca;">
                        <div class="">
                            <p><span class="tw-font-bold">Mode</span>: <span class="tw-text-gray-600 tw-capitalize">{{ listing.mode.label }} </span></p>
                    ...
                    <div class="tw-flex tw-justify-end">
                        <a :href="listing.website" @click="incrementClick(listing)" :name="'Redirect from ragnaranks to '+listing.website" target="_blank" class="at-btn tw-mr-2 tw-shadow at-btn--default at-btn--default--hollow at-btn__text"><i class="fas fa-globe-americas"></i></a>
                        <a :href="listing.route" :name="'View '+listing.name+' profile on Ragnaranks'" class="at-btn tw-shadow hover:tw-text-white at-btn--primary at-btn__text" style="display:flex">Explore</a>
                    </div>
                </div>
            </div>
        </div>
