        ...
        <div class="row no-gutters">
            <div class="col content tw-rounded tw-shadow-md">
                <div class="tw-flex tw-items-center tw-rounded-t" :class="'bg-'+$parent.$parent.$parent.accent+'-dark'">
                    <div class="tw-flex tw-flex-row tw-flex-1 tw-text-left">
                        <div class="info d-flex tw-flex-1 tw-flex-row tw-flex mb-1 tw-p-3">
                            <div
                                class="tw-rounded-full tw-h-10 tw-w-10 tw-flex tw-avatar-circle tw-items-center tw-justify-center tw-mr-3 tw-bg-white">
                                <img
                                    class="border hover:tw-border-solid hover:tw-border-grey hover:tw-shadow tw-rounded-full tw-avatar-circle tw-h-10 tw-w-10"
                                    :src="review.user.avatar" alt="">
                            </div>
                            <div class="tw-flex tw-flex-col">
                                <p class="tw-flex-1 tw-font-semibold tw-mb-0 tw-text-white" style="font-size:.9rem">{{
                ...
                <div v-show="viewingDetails" class="tw-p-6 tw-border-t tw-border-grey-light">
                    ...
                    <div class="tw-flex tw-flex-row">
                        <ul class="tw-flex-1 tw-mb-0">
                            <li class="tw-text-xs"><b>Donation Score</b>: {{ review.donation_score }}
                                ({{this.ratingScore(review.donation_score)}})
                            </li>
                            <li class="tw-text-xs"><b>Update Score</b>: {{ review.update_score }}
                                ({{this.ratingScore(review.update_score)}})
                            </li>
                            <li class="tw-text-xs"><b>Classes Score</b>: {{ review.class_score }}
                                ({{this.ratingScore(review.class_score)}})
                            </li>
                            <li class="tw-text-xs"><b>Items Score</b>: {{ review.item_score }}
                                ({{this.ratingScore(review.item_score)}})
                            </li>
                        </ul>
                        <ul class="tw-flex-1 tw-mb-0">
                            <li class="tw-text-xs"><b>Support Score</b>: {{ review.support_score }}
                                ({{this.ratingScore(review.support_score)}})
                            </li>
                            <li class="tw-text-xs"><b>Hosting Score</b>: {{ review.hosting_score }}
                                ({{this.ratingScore(review.hosting_score)}})
                            </li>
                            <li class="tw-text-xs"><b>Content Score</b>: {{ review.content_score }}
                                ({{this.ratingScore(review.content_score)}})
                            </li>
                            <li class="tw-text-xs"><b>Events Score</b>: {{ review.event_score }}
                                ({{this.ratingScore(review.event_score)}})
                            </li>
                ...
                <div v-show="commenting" class="tw-p-6 tw-border-t tw-border-grey-light">
                    ...
                    <at-textarea v-model="comment.message" min-rows="5" class="tw-mt-2" autosize
                                 placeholder="Write your reply towards this review"
                                 :class="validation.hasError('comment.message') ? 'invalid-textarea' : ''"></at-textarea>
                    <div v-if="validation.hasError('comment.message')"
                         class="tw-flex-1 tw-text-right help-block invalid-feedback">{{
                        validation.firstError('comment.message') }}
                    </div>
                    <at-button @click="postComment" type="error" class="tw-mt-2">Post Comment</at-button>
                </div>
                <div class="tw-border-t tw-border-gray-400 tw-py-2"
                     :class="'bg-'+$parent.$parent.$parent.accent+'-lighter'">
                    <div class="tw-flex tw-justify-end">
                        <at-button @click="viewingDetails = !viewingDetails" icon="icon-maximize-2" type="text">{{
                            detailButtonText }}
                        </at-button>
                        <at-button @click="reportReview" icon="icon-flag" type="text">Report</at-button>
                        <at-button v-if="$parent.$parent.$parent.isPageOwner() && review.comments.length === 0"
                                   @click="commenting = !commenting" icon="icon-paperclip" type="text">{{
                            commentButtonText }}
                        </at-button>
                    </div>
                </div>
