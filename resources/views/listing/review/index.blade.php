...
@section('meta_tags')
    ...
    <meta property="og:url" content="{{ ($listing->route . '/reviews') }}">
...
{{--                        </div>--}}
                        <div class="detail tw-flex tw-flex-row tw-flex-1 tw-items-center">
                            <div class="tw-flex-1">
                                <h2 class="text-capitalize tw-font-bold mb-0 tw-text-lg">{{ $review->user->username }}</h2>
                                <p class="text-black-50">Wrote {{ $review->created_at->diffForHumans() }}</p>
                            </div>
                            <div class="">
                    ...
                    <div class="tools">
                        <div class="tw-flex">
                            @if (auth()->check())
                                <button @click="report('{{ route('review.report.store', $review) }}')" class="tool-button text-dark"><i class="far fa-flag"></i> Report</button>
                            @can('delete', $review)
                            ...
                            @endcan
                            @else
                                <a href="{{ route('login') }}"><p class="text-black-50">Login to Interact</p></a>
                            @endif
                        </div>
                        <div class="tw-text-right">
                            <p @click="breakdown = !breakdown" class="tw-cursor-pointer text-dark" style="font-size:15px;">View Breakdown</p>
                        </div>
                    </div>
                    <div v-if="breakdown" class="tw-my-4 tw-bg-white at-table at-table--small at-table__content at-table__body">
